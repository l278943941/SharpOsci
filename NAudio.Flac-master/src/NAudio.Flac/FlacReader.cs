...
namespace NAudio.Flac
{
...

    {
        ...
        public int Read(float[] buffer, int offset, int count)
        {
            ...
            int totalBytesNeeded = count * bytesPerSample;

            // 临时缓冲区用于存储本次读取的字节数据
            byte[] byteBuffer = new byte[totalBytesNeeded];

            // 从基类 Read 方法读取字节数据
            int bytesRead = Read(byteBuffer, 0, totalBytesNeeded);
            ...
            // 将字节数据转换为浮点数组
            ConvertBytesToFloat(byteBuffer, 0, bytesRead, buffer, 0, bytesPerSample);
